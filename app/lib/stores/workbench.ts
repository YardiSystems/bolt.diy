...
export class WorkbenchStore {
  ...
  async syncFiles() {
    ...
    const files = this.files.get();
    const syncedFiles = [];
    const fileSaveRoot = (import.meta.env.HTTP_PROTOCOL || window.location.protocol) + '//' + (window.location.host || 'localhost') + (window.location.port ? ':' + window.location.port : '') + import.meta.env.VITE_FILESAVEROOT;
