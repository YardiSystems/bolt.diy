...
const ServiceStatusTab = () => {
  ...
  const getApiKey = useCallback(
    (provider: ProviderName): string | null => {
      ...
      if (provider === 'Anthropic' && apiKey) {
        const baseUrl = import.meta.env.ANTHROPIC_API_BASE_URL;
  ...
  const getProviderConfig = useCallback((provider: ProviderName): ProviderConfig | null => {
    ...
    const togetherBaseUrl = import.meta.env.ANTHROPIC_API_BASE_URL;

    if (provider === 'Anthropic' && togetherBaseUrl) {
      updatedConfig = {
        ...config,
        apiUrl: `${togetherBaseUrl}/messages`,
      };
    }
  ...
  const fetchPublicStatus = useCallback(
    async (
      ...
      try {
        ...
        switch (provider) {
          case 'Anthropic': {
            const endpointStatus = await checkEndpoint('https://status.anthropic.com/');
